...
def ping_host(ip, timeout=1):
    ...
    try:
        result = subprocess.run(
            ["ping", param, "1", timeout_param, str(int(timeout * 1000)) if platform.system().lower() == "windows" else str(int(timeout)), ip],
...
def main():
    ...
    parser.add_argument("-t", "--timeout", type=float, default=0.5, help="Connection timeout in seconds (default: 0.01)")
    ...
    ports = list(COMMON_PORTS.keys()) if args.common_ports else (args.port if args.port else None)
    if ports is None:
        console.print(f"[bold blue]Scanning {args.network} with ping (no ports specified) with timeout {args.timeout}s[/bold blue]")
    else:
    ...
    console.print(f"[bold green]Scan completed in {time.time() - start:.2f} seconds[/bold green]")
